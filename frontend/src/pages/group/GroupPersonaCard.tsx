...
export default function GroupPersonaCard({ groupId }: { groupId: number }) {
  const [data, setData] = useState<GroupPersona | null>(null);
  const [loaded, setLoaded] = useState(false);

  useEffect(() => {
    let active = true;
    getGroupPersona(groupId)
  ...
  if (!loaded) return <div className="mb-3.5 h-20 animate-pulse rounded-card border border-border bg-surface" />;
  if (!data) return null;
  ...
  return (
    <div className="mb-3.5 rounded-card border border-border bg-surface p-3.5">
      ...
      {data.conflictMessage && (
        <p className="mt-2 flex items-start gap-1.5 rounded-lg bg-primary/10 px-2.5 py-1.5 text-[12px] font-semibold text-[#B5763E]">
